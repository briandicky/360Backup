...
int main(int argc, char *argv[])
{
	...
	double last_end=0.0, start_time=0.0, end_time=0.0, airtime=0.0, total_time=0.0;
    unsigned int total_file = 0;
    double prev_time = 0.0, diff_time = 0.0;
    ...
    printf("frame_no,frame_size,src_addr,dst_addr,arrival_time,diff_time\n");
	...
	while (int returnValue = pcap_next_ex(pcap, &header, &packet) >= 0)
	{
		...
		char arrtimec[30];
		printf(arrtimec, "%d.%d", header->ts.tv_sec, header->ts.tv_usec);
    ...
    printf("#----------------------------------------\n");
	printf("total sample time: %f secs\n", total_time);
