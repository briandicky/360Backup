...
cgt_yaw=[]
cgt_pitch=[]
outgt.write("no., tile numbers\n")
outs.write("no., mapped x, mapped y, mapped z, mapped yaw, mapped pitch, mapped roll, raw x, raw y, raw z, raw yaw, raw pitch, raw roll, mapped corr yaw, mapped corr pitch, mapped corr roll, raw corr yaw, raw corr pitch, raw corr roll\n")
...
#		print pitch
	if curtime<sctime+ctime-5:
		...
		while curtime>=nexttime:
			...
			#print m, curtime
			outs.write(video+"_%05d.png"%(m)+", "+str(mx)+", "+str(my)+", "+str(mz)+", "+str(myaw)+", "+str(mpitch)+", "+str(mroll)+", "+str(x)+" ,"+str(y)+", "+str(z)+", "+str(yaw)+", "+str(pitch)+", "+str(roll)+", "+str(mtmpgty)+", "+str(mtmpgtx)+", "+str(mroll)+", "+str(tmpgty)+", "+str(tmpgtx)+", "+str(roll)+"\n")
...
#			print yaw, pitch, x, y
	...
	outgt.write(video+'_%05d.png, '%(i+1))
	for j in range(0, len(tiles)):
		if j!=0:
			outgt.write(" ")
		outgt.write(str(tiles[j]))
	outgt.write("\n")
outgt.close()
outs.close()
#tmpw.close()
